...
const GoogleCalendarIntegration = ({ user, onUpdateIntegrations }) => {
  ...
  const API_BASE_URL = process.env.REACT_APP_API_URL || 'https://taskflow-enlep7oc.b4a.run/'; // Empty string for same-origin
    ...
    if (key === "calendar") {
      ...
      if (!integrations.calendar) {
        ...
        try {
          ...
          await axios.get(`https://taskflow-enlep7oc.b4a.run/api/health`);
  ...
  const connectGoogleCalendar = () => {
    ...
    window.location.href = `https://taskflow-enlep7oc.b4a.run/auth/google?userId=${user.id}`;
  };
  ...
  const disconnectGoogleCalendar = async () => {
    ...
    try {
      const response = await axios.post(`https://taskflow-enlep7oc.b4a.run/api/disconnect-google-calendar`, { userId: user.id });
