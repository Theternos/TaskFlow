  ...
  const fetchCancellationRequestStatus = async () => {
    ...
    if ((requestStage === 'success' && activeSection === 'cancellation') || cancellationRequestSubmitted) {
      try {
        ...
        const response = await axios.get(`https://taskflow-enlep7oc.b4a.run/api/cancellation-requests/${task.id}`);
  ...
  const fetchRequestStatus = async () => {
    ...
    if (requestStage === 'success' || requestSubmitted) {
      try {
        ...
        const response = await axios.get(`https://taskflow-enlep7oc.b4a.run/api/extension-requests/${task.id}`);
  ...
  useEffect(() => {
    ...
    const checkExistingRequests = async () => {
      try {
        ...
        const extensionResponse = await axios.get(`https://taskflow-enlep7oc.b4a.run/api/extension-requests/${task.id}`);
        ...
        const cancellationResponse = await axios.get(`https://taskflow-enlep7oc.b4a.run/api/cancellation-requests/${task.id}`);
    ...
    try {
      ...
      if (extensionFile) {
        ...
        response = await axios.post(
          `https://taskflow-enlep7oc.b4a.run/api/tasks/${task.id}/extension-request`,
          formData,
          {
            headers: {
  ...
  const handleCancellationRequest = async () => {
    ...
    try {
      ...
      if (cancellationFile) {
        ...
        response = await axios.post(
          `https://taskflow-enlep7oc.b4a.run/api/tasks/${task.id}/cancellation-request`,
          formData,
          {
            headers: {
              'Content-Type': 'multipart/form-data',
