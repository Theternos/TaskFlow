...
const StaffRequests = () => {
  ...
  const fetchRequestHistory = async (currentUser) => {
    try {
      ...
      // Fallback: Simulate request history from tasks with extension/cancellation requests
      simulateRequestHistory(currentUser);
    }
  };

  // Simulate request history if API endpoint doesn't exist yet
  const simulateRequestHistory = (currentUser) => {
    // Create sample data if the API call fails or there's no data
    if (
      !tasks.length ||
      !tasks.some(
        (task) =>
          (task.extensionRequests && task.extensionRequests.length > 0) ||
          (task.cancellationRequests && task.cancellationRequests.length > 0)
      )
    ) {
      // Generate sample data if no real requests exist
      const sampleHistory = [
        {
          taskId: "sample-1",
          taskTitle: "Sample Task 1",
          taskDescription:
            "This is a sample task description for demonstration",
          requestType: "extension",
          reason: "Need additional time to complete requirements",
        ...
        },
        {
          ...
          reason: "Task is no longer required due to changed priorities",
          requestDate: new Date(
    ...
    // Original implementation for when there is data
    const history = tasks
      .filter(
        (task) =>
          (task.extensionRequests && task.extensionRequests.length > 0) ||
      ...
      .map((task) => {
        // Combine extension and cancellation requests
        const requests = [
          ...
          ...(task.cancellationRequests || []).map((req) => ({
            ...req,
            taskId: task.id,
            taskTitle: task.title,
            taskDescription: task.description,
  ...
  const fetchTasks = async (currentUser) => {
    ...
    try {
      ...
      fetchRequestHistory(currentUser);
    } catch (err) {
  ...
  const prioritizeTasks = (tasks) => {
    return tasks
      .map((task) => {
        ...
        // Adjust score based on deadline proximity
        if (daysUntilDeadline <= 3) {
          priorityScore += 40; // Urgent tasks get a big boost
        } else if (daysUntilDeadline <= 7) {
          priorityScore += 20; // Near-term tasks get a moderate boost
        } else if (daysUntilDeadline <= 14) {
          priorityScore += 10; // Upcoming tasks get a small boost
        }

        // Additional factors
        if (task.status === "Rework") {
          priorityScore += 25; // Rework tasks get higher priority
        }
        ...
        if (task.status === "Completed") {
          priorityScore -= priorityScore; // Completed tasks get a lower priority
        }

        if (task.status === "Progress") {
          priorityScore -= priorityScore * 0.7; // Tasks in progress get a lower priority
        }
      ...
      .sort((a, b) => b.priorityScore - a.priorityScore); // Sort in descending order of priority
  };
                    ...
                    <table className="task-table">
                      ...
                      <tbody>
                        {filteredTasks.map((task) => {
                          // Check if the task's status is "Pending" or "Rework" 
                          // and if the last cancellation request status is NOT "Approved"
                          const isApproved = task.cancellationRequests?.[task.cancellationRequests.length - 1]?.status === "Approved";

                          if ((task.status === "Pending" || task.status === "Rework") && !isApproved) {
                            return (
                              <tr key={task.id}>
                                <td>
                                ...
                                <td>
                                  <div className={`status-badge ${task.status?.toLowerCase() || "pending"}`}>
                                    {" "}
                                    &nbsp;
                                ...
                                <td>
                                  <button className="open-request-modal-btn" onClick={() => openRequestModal(task)}>
                          ...
                          // Return null if the task shouldn't be displayed
                          return null;
                        })}
                      </tbody>
