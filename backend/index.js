...
const sendTaskNotification = async (userEmail, taskDetails, userName) => {
  try {
    const mailOptions = {
      from: 'kavin.apm2003@gmail.com', // replace with your email
      to: userEmail,
      ...
      html: `
        ...
        <div style="margin: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 5px;">
          ...
          ${taskDetails.tags ? `<p><strong>Tags:</strong> ${taskDetails.tags.join(', ')}</p>` : ''}
        </div>
...
// Function to Send Rework Email Notification 
const sendReworkNotification = async (userEmail, task, comment, deadline, userName) => {
  try {
    const mailOptions = {
      ...
      html: `
        <h2>Task Rework Required</h2>
        <p>Dear ${userName},</p>
        <p>The following task requires rework. Please review the details:</p>
        <div style="margin: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 5px;">
          <p><strong>Title:</strong> ${task.title}</p>
          <p><strong>Priority:</strong> ${task.priority || 'Normal'}</p>
          <p><strong>Status:</strong> ${task.status}</p>
          <p><strong>Rework Comment:</strong> ${comment}</p>
          <p><strong>Rework Deadline:</strong> ${deadline}</p>
        </div>
        <p>Please log in to your dashboard to make the required changes.</p>
        <p>Thank you,<br>Task Management System</p>
      `
    };
...
// 1. First, let's create a function to send reminder emails
const sendTaskReminder = async (userEmail, taskDetails, userName, isRework = false, reworkIndex = null) => {
  try {
    const subject = isRework 
      ? `REMINDER: Rework Due Tomorrow - ${taskDetails.title}`
      : `REMINDER: Task Due Tomorrow - ${taskDetails.title}`;
    ...
    const mailOptions = {
      ...
      subject: subject,
      html: `
        <h2>${isRework ? 'Rework Reminder' : 'Task Reminder'}</h2>
        <p>Dear ${userName},</p>
        <p>This is a friendly reminder that the following ${isRework ? 'rework' : 'task'} is due tomorrow:</p>
        <div style="margin: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;">
          <p><strong>Title:</strong> ${taskDetails.title}</p>
          ${taskDetails.description ? `<p><strong>Description:</strong> ${taskDetails.description}</p>` : ''}
          <p><strong>Status:</strong> ${taskDetails.status}</p>
          <p><strong>Priority:</strong> ${taskDetails.priority || 'Medium'}</p>
          <p><strong>Due Date:</strong> ${isRework ? taskDetails.reworkDeadline : taskDetails.dueDate}</p>
          ${taskDetails.tags ? `<p><strong>Tags:</strong> ${taskDetails.tags.join(', ')}</p>` : ''}
          ${isRework && taskDetails.reworkComment ? `<p><strong>Rework Comment:</strong> ${taskDetails.reworkComment}</p>` : ''}
        </div>
        <p>Please ensure you complete this ${isRework ? 'rework' : 'task'} before the deadline.</p>
        <p>Thank you,<br>Task Management System</p>
      `
    };

    const info = await transporter.sendMail(mailOptions);
    console.log(`${isRework ? 'Rework' : 'Task'} reminder email sent successfully:`, info.response);
    return true;
  } catch (error) {
    console.error(`Error sending ${isRework ? 'rework' : 'task'} reminder email:`, error);
    return false;
  }
};
